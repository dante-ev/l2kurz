\section{Eingabefile}
Das Eingabefile für \LaTeX{} ist ein Textfile mit der Endung \lstinline+.tex+. Es 
...
\todo{MD: Sollte hier ein Hinweis auf Editoren gegeben werden?
bspw: \href{http://tex.stackexchange.com/questions/339/latex-editors-ides}%
{LaTeX Editors/IDEs}\\PG: ich würde erwähnen, dass es spezielle Editoren dafür
 gibt, z.B. (einen erwähnen, wie beispielsweise TeXmaker)}
...
\todo{MD: Ich finde Ziffer verwirrt irgendwie?\\
     PG: ja, Ziffer können wir rausnehmen. Hier und im Satz davor.}
Ziffer. Groß- und Kleinbuchstaben haben auch in Befehlsnamen \emph{verschiedene} 
Bedeutung. Wenn man nach einem Befehlsnamen eine Leerstelle erhalten will, muss 
man~\lstinline|{}| zur Beendigung des Befehlsnamens oder einen eigenen Befehl für die
...
\subsection{Aufbau}
 \todo[inline]{MD: Erste Überarbeitung begonnen}
...
\sout{Im \local\ können weitere verfügbare Klassen angegeben sein.}
\todo{MD: M.E. Raus damit}
...
\todo[inline]{MD: Jetzt sollten wir uns auf Pakete einigen. Habe schon xcolor 
              statt color und imakeidx statt makeidx genutzt. Ich denke wenn 
              fancyhdr angebenen ist, sollte auch scrpage2 dabei sein.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{example}[caption={Bestimmung der Kodierung für die Nutzung von Umlauten},]
 ...
 \usepackage[T1]{fontenc}%Umlaute als einzelnes Zeichen
\end{example}

\noindent\colorbox{gray}{%
\parbox{\dimexpr\linewidth-2\fboxsep\relax}{%
\todo[inline]{MD: Alter Text}
Damit auch diese Zeichen im Eingabefile benutzt werden dürfen,  muss 
man das Paket \texttt{inputenc} laden und dabei die jeweilige Kodierung als 
Option angeben: \texttt{\string\usepackage[\textit{codepage}]\{inputenc\}}.
Mögliche Angaben für \textit{codepage} sind u.\,a.:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo[inline]{MD: hier sollte auf selinput verwiesen werden. Der Nutzer braucht nicht
