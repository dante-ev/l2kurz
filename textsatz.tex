...
sollte allerdings explizit angesprochen werden.}
...
\todo{MD: Warnmeldung ist richtig. Wobei bis dato noch nicht auf Fehler/Warnungen eingegangen wurde}
...
Mit \verb|\,| lässt sich ein kurzer Abstand erzeugen, wie er\todo{MD: Genannt Spatium}
...
angegeben werden.
Mit dem Paket \texttt{german} \todo{MD: überarbeiten} ist \verb:\frenchspacing:
...
\todo{MD: Ich finde ein Hinweis auf \href{http://zvisionwelt.files.wordpress.com/2012/01/typokurz.pdf}{typokurz} wäre hilfreich}
...
\end{verbatim}
\exc
Mit dem Paket \texttt{german} steht zusätzlich der\todo{MD: überarbeiten}
...
\todo{MD: Sollte \texttt{\string\part} ergänzt werden?}
...
ist der Befehl \verb|\footnote| nicht erlaubt.\todo{MD: Aber es gibt Möglichkeiten}
\exa
~
\exb
\begin{verbatim}
...
\subsubsection{Listen (itemize, enumerate, description)}
 \todo{MD: Hier empfehle ich das Paket enumitem}
...
die Mitte der Zeile\todo{PG: ragged2e erwähnen? -> besserer Umbruch\\MD: Bin mir unschlüssig. Besonders erachte ich \textbackslash\textbackslash als falsch.}.
...
und \verb|\\| trennt die Zeilen.\todo{MD: Sehr wichtig: Seitenumbruch möglich}
%
...
bestimmt\todo{MD: Erachte ich als falsch. Sollte genauer auf Spaltentypen eingegangen werden}
