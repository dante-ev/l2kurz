...
\todo[inline,color=done]{PG: eingefügt}
...
Innerhalb von Gleitobjekten (siehe Abschnitt~\ref{floats}), 
...
(siehe Abbildung~\vref{desc}). \todo[color=done]{PG: eingefügt}Mit dem Paket \texttt{enumitem} können die Umgebungen leicht den eigenen Bedürfnissen angepasst werden.
...
\todo[inline,color=done]{PG: überarbeitet - bitte kontrollieren}
...
werden, indem man sie zwischen \lstinline.\lstinline|. und~\lstinline.|.\todo{PG: naja, lstinline gilt ja nur im Prinzip für Programmcode. Hier würde ich schon verb verwenden und auf \cs{lstinline} verweisen /erwähnen}
einschließt.
...
%\todo{PG: booktabs mit \cs{toprule},\cs{midrule} und \cs{bottomrule} erwähnen? - macht eine schönere Tabelle\\MD: Ein wenig Typographie gehört zur Einführung}.
